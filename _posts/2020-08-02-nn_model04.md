...
### 判别器  
1. 目标$maxV(D)=E_{x\sim p_r(x)}[log(D(x))] + E_{z\sim p_z(z)}[log(1-D(x))]$

2. $D(x)$越大越好  
反映在loss中，真样本经d判别，结果与1越接近，d就越优秀，损失越小；

3. $1-D(x)$越大越好，则$D(x)$越小越好  
反映在loss中，假样本经d判别，判断结果与0越接近，d就越优秀，损失越小

### 生成器  
1. 目标$minV(G)=E_{x\sim p_g(x)}[log(1-D(x))]$
...
### 缺陷推导
生成器目标改进$minV(G)=E_{x\sim p_g(x)}[log(1-D(x))] \to minV(G)=E_{x\sim p_g(x)}[-logD(x)],(1)$
